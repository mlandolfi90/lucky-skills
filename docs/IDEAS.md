...
- 2026-07-16 · DEFECTO REAL en `scripts/leak-scan.sh:61` (regla RUTA-ABSOLUTA), hallado por el leak-verifier de la corrida equipo-doc-v1: la rama Windows del regex está MUERTA por DOBLE-ESCAPE — el ERE exige DOS backslashes literales donde un path real trae UNO, así que un path Windows real NO matchea. Probado end-to-end con el script REAL en repo temporal: path Windows de otro usuario (backslashes simples) → exit 0 (pasa en verde); path `/home/<otro>/` → exit 1. Hoy el leak de path Windows solo se atrapa de rebote por la regla 2 (nombre del operador hardcodeado): un path de OTRO usuario se filtraría EN SILENCIO. Fix: un nivel de escape menos en las tres alternativas del patrón. Escalón sugerido: microfix (1 comportamiento, 1 punto) con prueba negativa obligatoria (el gate debe VERSE morder). NOTA 1: el PASS de ZERO_LEAK de esa corrida NO dependía de este regex — el verificador lo suplió con barrido independiente sobre 182 archivos. NOTA 2 (lección v1.8.0, mordida en vivo): documentar este defecto CITANDO el regex literal hizo que el propio scanner se disparara sobre esta línea y abortara la forja. Al documentar un patrón, usar SIEMPRE placeholders `<asi>` — nunca el literal · corrida equipo-doc-v1 · **PROMOVIDA 2026-07-16 → `diagnostico:2026-07-16-leak-scan-ruta-windows-muerta` (ABIERTO)** — el hallazgo con su evidencia, hipótesis y escalón vive allá; acá queda el puntero.
- 2026-07-16 · FASE PIN como paso 0 de todo tier completo con >1 carril (RETRO de equipo-doc-v1, que gastó 2 de 3 iteraciones en esto): fijar los CONTRATOS cross-carril ANTES de mandar a planificar, en vez de descubrir el ciclo con el primer REJECT del Steward. Los sets de archivos pueden ser disjuntos (cero colisión física) y aun así los planes forman un CICLO de contratos (A necesita el formato de C · C la forma de B · B el nombre de A) que ningún orden de carriles resuelve. El COLLISION-MAP hoy caza colisiones de ARCHIVO, no de CONTRATO ni de REFERENCIA · corrida equipo-doc-v1 · **PROMOVIDA 2026-07-16 → saber `CAND-6b15ab5f60b1`** (rama `mcp-inbox/84a3c1a8326ab913`, PENDIENTE de tu merge y de tu promoción a LIVE).
- 2026-07-16 · Citar código por NÚMERO DE LÍNEA entre carriles paralelos es intrínsecamente frágil (RETRO de equipo-doc-v1: mató la corrida en el último paso): el carril vecino inserta una línea y tu ley SELLADA queda mintiendo — y una ley que miente invita al mantenedor a desarmar la regla que la cita sostenía. Candidato a regla de la propia skill crisol y a entrada de bitácora (síntoma: "la cita era correcta contra HEAD y falsa contra el árbol staged"). Antídoto propuesto: citar por ANCLA DE TEXTO (la regla, el nombre, la frase) en vez de por número, o declarar el riesgo explícito cuando el número es inevitable · corrida equipo-doc-v1 · **PROMOVIDA 2026-07-16 → saber `CAND-2679c3f1ec6f`** (rama `mcp-inbox/79e206bfca68cd52`, PENDIENTE de tu merge y de tu promoción a LIVE).
- 2026-07-17 · [corrida el-verde-significa-algo] «Auditá lo que el CÓDIGO hace, no lo que el comentario promete» (bitacora:DRIFT-001) al prompt canónico del Verificador — es el NEXT abierto de DRIFT-001.md:22 ("queda decidir si va a regla del Verificador"), territorio de REGLA 0 (paso 3, verificación funcional), NO de RED_GREEN. Sacada IN-FLIGHT por el Steward: fuera del Alcance CERRADO de esta corrida. Su argumento, que vale como doctrina: si "restatea una prevención ya vinculante" bastara como pasaporte, toda entrada de la bitácora sería injertable en todo prompt canónico de toda corrida cuyo TEMA roce — y "Alcance (CERRADO)" dejaría de significar algo; una corrida no puede cerrar un NEXT abierto de la bitácora de refilón, sin ADR y sin el operador (precedente del movimiento correcto: TEST_COVERAGE: NONE, ADR 0022 §3 + PIN 3). Para la corrida que toque el prompt de REGLA 0 · decisión del operador
- 2026-07-17 · HALLAZGO DEL CIERRE (Fase 1 del gate, no está escrito en ninguna parte de la ley): el commit de cierre de una corrida que toca CÓDIGO exige que la fila siga `estado: ACTIVE` con `runState: closing` — ése es el momento en que el gate audita la matriz (`crisol_gate.py:682-688`: "el commit de cierre (runState: closing) exige la matriz completa+verde"). Si la fila se pasa a CLOSED ANTES de commitear, el gate va a buscar la corrida para auditarle la matriz, no encuentra ninguna (`_ledger_state` → NONE) y BLOQUEA SU PROPIO CIERRE. Orden correcto: `runState: closing` + ACTIVE → commit (el gate audita) → `estado: CLOSED` en un commit de docs, exento. SON DOS COMMITS. Nadie lo sabía porque ninguna corrida previa había cerrado con código staged que el gate clasifique como fuente (`_CODE_EXTS` incluye .yml/.sh/.py; .md está excluido, y hasta hoy los cierres eran casi todo .md). Segundo mordisco relacionado: tras el commit, editar la fila YA SELLADA por la forja dispara "sello ROTO (M8)" y la forja ABORTA — la lección corolario es que el aprendizaje del cierre va al parking o al saber, JAMÁS a la fila sellada. Candidato a rama de la skill crisol o a línea en el paso 8 del §4 · corrida el-verde-significa-algo
- 2026-07-17 · saber: propuesta pendiente mcp-inbox/1cfc23a112e436d4 (CAND-656b78f9f331) · cierre de corrida con código staged = DOS commits (closing→CLOSED; sello ROTO M8) · endosar con /saber revisar
- 2026-07-17 · saber: propuesta pendiente mcp-inbox/3c7209ea14ca51e9 (CAND-51bb77172c08) · verificá el artefacto que VIAJA (el índice), no el que tenés a mano (el disco) · endosar con /saber revisar
...
- 2026-07-18 · SKILL-REVIEW PROPIO (auditor de calidad de las PROPIAS skills de lucky, con DIENTE) — lo más valioso (>8) del repo sanyuan-skills (github.com/sanyuan0704/sanyuan-skills, MIT). Hoy nadie audita una skill de lucky COMO artefacto: registros-lint valida forma del frontmatter de agentes/tablas, test-ruteo cubre 1 sola dimensión (description con disparadores); las otras 4 no las juzga nadie. Semilla = las 5 dimensiones de su skill-review: (1) estructura/layout + SKILL.md acotado, (2) description con disparadores rutéables + keyword-bombing, (3) diseño del workflow (checklist, gates de confirmación, pasos REQUIRED/BLOCKING), (4) eficiencia de tokens (progressive loading de references, imperativo, cero redundancia), (5) anti-patterns (directivas vagas, placeholder residue, over-spec de lo que Claude ya sabe). SE COSECHA LA ESTRUCTURA (las 5 dimensiones), NO SU PROSA: skill-review de sanyuan es persuasivo (reporta suggestions, no gatea) y su quick_validate.py rechaza disable-model-invocation (key que lucky usa) → NO adoptable tal cual. Vehículo lucky = feature/corrida con VEREDICTO (verificador fresco a la manera del roster), no consejo. Por qué >8: una forja que forja skills debería poder auditar la calidad de las que produce — gap meta-nivel real. Requiere endoso del operador ítem por ítem antes de nacer · análisis sanyuan-skills 2026-07-18
- 2026-07-18 · ENMIENDA a la receta [DECIDIDO 2026-07-06] de impeccable: el `npx impeccable link` sin versión también es floating (pin de identidad ≠ pin de ejecución) — corregido en diseno/SKILL.md y cláusula nueva en crisol §Pin total + test-pin-scan.sh · cosecha agent-skills, endoso del operador
- 2026-07-19 · PODA hotfix §4: el aterrizaje por ecosistema quedó con DOS hogares (rama 004 §Cómo-se-expresa y hotfix §4) — anticipado por el supuesto 4 del plan versionado-artefactos y observación no bloqueante del design-verifier; consolidar en uno cuando toque editar hotfix · corrida 2026-07-19-versionado-artefactos
- 2026-07-24 · FOLLOW-UP limpieza Fase 2: 4 referencias stale al modelo viejo (CANDIDATE/promover) fuera del alcance de la corrida saber-fase2, benignas/fail-open — bitacora/hooks/bitacora-push.sh:168,176 (hook: "promover a LIVE"/"ficha CANDIDATE"), cumplimiento/SKILL.md:65 + cumplimiento/templates/reporte.md:27 ("evidencia→CANDIDATE→endoso"), arquitectura/templates/auditoria-solid.md:92 ("entrada CANDIDATE tipo DRIFT"). Reconciliar al modelo captura→LIVE (ADR 0028) en un microfix de limpieza · declarado por el scope-verifier de la corrida
