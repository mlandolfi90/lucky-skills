...
- 2026-07-16 · DEFECTO REAL en `scripts/leak-scan.sh:61` (regla RUTA-ABSOLUTA), hallado por el leak-verifier de la corrida equipo-doc-v1: la rama Windows del regex está MUERTA por doble-escape — en ERE `C:\\Users\\` exige DOS backslashes literales, así que un path Windows real (un solo backslash) NO matchea. Probado end-to-end con el script REAL en repo temporal: path Windows de otro usuario → exit 0 (pasa en verde); path `/home/<otro>/` → exit 1. Hoy el leak de path Windows solo se atrapa de rebote por la regla 2 (nombre del operador hardcodeado): un path de OTRO usuario se filtraría EN SILENCIO. Fix: `\` en vez de `\\` en las tres alternativas del patrón. Escalón sugerido: microfix (1 comportamiento, 1 punto) con prueba negativa obligatoria (el gate debe VERSE morder). NOTA: el PASS de ZERO_LEAK de esa corrida NO dependía de este regex — el verificador lo suplió con barrido independiente sobre 182 archivos · corrida equipo-doc-v1
